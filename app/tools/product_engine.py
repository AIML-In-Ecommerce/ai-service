...
product_engine = FunctionTool.from_defaults(
    ...
    name  = "product_getter",
    description="this tool can get list product by keyword, parameter request_param_str is a URL's query string which provide additional information include keyword (the keyword of product being found bu user), quantity (the number of product user want to receive), sortBy ({{soldQuantity}} in case related to to number of sold product and {{avgRating}} in case related to rating value). Example: {{request_param_str: '?keyword=ao-thun?quantity=2?sortBy=soldQuantity'}}  It's not necessary to always have all three request parameters. For example, '?shop=123456789&year=2024&month=7'",
)